...
class ZetTruyen : HttpSource() {
    ...
    override val baseUrl = "https://www.zettruyen.work"
    ...
    private fun mangaFromElement(element: Element) = SManga.create().apply {
        ...
        thumbnail_url = element.selectFirst("img")?.absUrl("src")?.useWorkingImageCdn()
    }
    ...
    override fun getFilterList(): FilterList {
        fetchGenres()
        return getFilters(genreList.ifEmpty { getGenres() })
    }
    ...
    private fun fetchGenres() {
        ...
        runCatching {
            client.newCall(GET("$baseUrl/the-loai", headers)).execute().use { response ->
                response.asJsoup()
                    .select("a.block[href*=/the-loai/]")
                    .mapNotNull { element ->
                        val slug = element.absUrl("href")
                            .substringAfter("/the-loai/", "")
                            .substringBefore("?")
                            .substringBefore("#")
                            .trim('/')
                            .takeIf { it.isNotBlank() }
                            ?: return@mapNotNull null
                        val name = element.text().replace(WHITESPACE_REGEX, " ").trim()
                        if (name.isBlank()) null else Genre(name, slug)
                    }
                    .distinctBy { it.id }
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        return SManga.create().apply {
            ...
            thumbnail_url = document.selectFirst("img[src*=/thumb/]")?.absUrl("src")?.useWorkingImageCdn()
    ...
    private fun String.useWorkingImageCdn(): String = replace(BROKEN_IMAGE_CDN, WORKING_IMAGE_CDN)

    companion object {
        private const val BROKEN_IMAGE_CDN = "https://cdn3.zetimage.com/"
        private const val WORKING_IMAGE_CDN = "https://cdn2.zetimage.com/"
